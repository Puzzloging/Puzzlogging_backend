...
# 포토모자이크 출력 해상도(10~50 사이의 최적값) 값이 높을수록 저해상도 포토모자이크(파일 크기 작음), 값이 작을수록 고해상도 포토모자이크(파일 크기가 큼)로 나타납니다.
...
# 모자이크 타일로 사용될 개별 필러 이미지의 해상도(픽셀 단위)(최적 값은 50~100 사이)
...
parser.add_argument("--member", type=str, help="member")
...
def image_stitch():
    ...
    cv2.imwrite ("mosaic.jpg", photomosaic)
    if args.verbose: time.sleep(sdelay)
...
print ("\nStart")
initialise()                #폴더 초기화
split_main_image()          #메인 사진 자르기
rename_filler_images()      #필러 사진 이름 바꾸기
crop_filler_images()        #필러 사진 정사각형으로 자르기
resize_filler_images()      #설정한 사이즈로 resize
avg_rgb_filler_images()     #각 필러 사진의 한 픽셀당 rgb의 평균을 구해서 filler_img_details에 추가
avg_rgb_main_image_blocks() #메인 사진을 자른 각 image_block 사진의 한 픽셀당 rgb의 평균을 구해서 main_img_block_details에 추가
image_match()               #각 image_block당 가장 가까운 색의 필러 사진을 선택
image_stitch()              #선택된 필러 이미지를 붙임
